#!/usr/bin/env python3
"""Unified four-method evaluation of the completed MVTec AD 15-class maps (A76).

A76 supersedes A73 for the paper main table: MVTec AD has 15 categories and cable
is one of them, so cable is evaluated inside the same protocol instead of being
reported as a separate single-category transfer experiment.
Component maps are read from the A73 map directory, which now also contains the
cable addendum generated under the identical 448/672 protocol.
"""
...
OUT = ROOT / "orbitad/results/a76_mvtec15_unified_eval_v1"
CATEGORIES = (
...
# Cable is a MVTec AD category (15 in total) but its component maps come from the
# A59/A71 legacy runs, whose directory layout differs from A73. A grid-pitch check
# on the stored maps gives 21.17 px for cable tiled and 31.75 px for cable global,
# identical to the certified A73 reference (672 and 448 at 1024 px), so cable's
# maps are read in place instead of being regenerated.
LEGACY_MAP_ROOTS = {
    "cable": {
...
def component_paths(split: str, category: str, anomaly_type: str, stem: str):
    if category in LEGACY_MAP_ROOTS:
        root = LEGACY_MAP_ROOTS[category][split]
...
def main():
    ...
    (OUT / "A76_UNIFIED_EVALUATION_COMPLETE.json").write_text(json.dumps({"status": "complete"}, indent=2))
