...
R=Path('/root/private_data/iad-vlm-anomaly');O=R/'model_comparison/efficientad_export_matched';O.mkdir(parents=True,exist_ok=True)
...
for cat in ['fruit_jelly','sheet_metal','vial','walnuts']:
 ...
 for i,(sha,(p,label)) in enumerate(originals.items()):
  with torch.inference_mode():
   ...
   x=model.pre_processor.transform(x).cuda()
   with torch.autocast(device_type="cuda",dtype=torch.float16): raw=model.model(x)
...
(O/'complete.json').write_text(json.dumps(dict(training=False,inference='fp16 autocast, test callback transform (antialias=True), checkpoint model; raw score retained',categories=meta),indent=2))
