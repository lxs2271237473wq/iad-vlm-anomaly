...
BASE=ROOT/'results/stage25_tiny_defects';OUT=BASE/'dinov2_validation_v1'

@torch.inference_mode()
def main():
    OUT.mkdir(parents=True,exist_ok=False);seed_all(42);torch.set_num_threads(4)
    (OUT/'protocol.json').write_text(json.dumps(dict(code_sha256=sha256(__file__),model='facebook/dinov2-base',input_size=518,reference_count=32,reference_selection='First 32 normal train image IDs per category; every patch retained',feature='L2 normalized last hidden patch tokens, exclude CLS',primary='max cosine nearest-normal distance',secondary='mean largest 1% patch distances',scope='300 source_validation images; fusion fit only on eight source_fit categories',training=False,limitations='New backbone baseline only, not novel algorithm or exact AnomalyDINO reproduction; reference budget differs from original WRN50.'),indent=2))
    ...
    print('FUSION_FROZEN',flush=True)
    ...
    manifest=pd.read_csv(BASE/'v1/inference_manifest.csv');evals=manifest[manifest.role=='source_validation'];assert len(evals)==300
    rows=[]
    ...
    rows=[];cis=[]
    for scope,data in [('source_validation',f),('tiny_only_plus_all_normal',f[(f.label==0)|(f.stratum=='tiny_only')])]:
