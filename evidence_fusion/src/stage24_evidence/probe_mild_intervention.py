...
def main():
    ...
    selected=source.sort_values('image_id').groupby(['category','label']).head(16)
    assert len(selected)==384 and selected.category.nunique()==12
    selected.to_csv(OUT/'selected_images.csv',index=False)
    protocol=dict(code_sha256=sha256(Path(__file__)),images=384,views=1920,selection='first sixteen image IDs per source category and image label',factors=[.9,1.1],head_seed=42,target_used=False,limitations='Exploratory small source subset; masks diagnostic only; image transforms need not preserve defect visibility; no causal or novelty claim.')
    ...
    for category,g in selected.groupby('category',sort=True):
        ...
        for r in g.itertuples():
            ...
            views=[('identity',im)]+[(f'{kind}_{factor}',enhancer(im).enhance(factor)) for kind,enhancer in [('brightness',ImageEnhance.Brightness),('contrast',ImageEnhance.Contrast)] for factor in [.9,1.1]]
