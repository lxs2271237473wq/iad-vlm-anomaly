...
Industrial anomalies often occupy small localized regions and may not dominate global image semantics. This makes direct full-image VLM reasoning unreliable for industrial inspection. A detector can provide localization evidence, while a VLM can provide visual-language abnormality evidence; however, naive fusion does not model whether the localized crop is reliable. We address this gap by calibrating crop-level VLM scores with candidate quality derived from localization evidence.
...
Full-image VLM obtains mean AUROC `0.6459`, while context-aware VLM obtains `0.7101`. PatchCore obtains `0.7853` and EfficientAD-30 fixed-budget obtains `0.7604`. The fair LOCO fusion reaches `0.8210`, improving over PatchCore by `+0.0356` and over EfficientAD-30 by `+0.0606`. The same-set result `0.8453` is reported only as an upper-bound diagnostic.
