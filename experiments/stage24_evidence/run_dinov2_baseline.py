...
BASE=ROOT/'results/stage25_tiny_defects';OUT=BASE/'dinov2_baseline_v1'

@torch.inference_mode()
def main():
    OUT.mkdir(parents=True,exist_ok=False);seed_all(42);torch.set_num_threads(4)
    (OUT/'protocol.json').write_text(json.dumps(dict(code_sha256=sha256(__file__),model='facebook/dinov2-base',input_size=518,reference_count=32,reference_selection='First 32 normal train image IDs per category; every patch retained',feature='L2 normalized last hidden patch tokens, exclude CLS',primary='max cosine nearest-normal distance',secondary='mean largest 1% patch distances',scope='All 1462 source_fit images; validation/holdout/AD2 not evaluated',training=False,limitations='New backbone baseline only, not novel algorithm or exact AnomalyDINO reproduction; reference budget differs from original WRN50.'),indent=2))
    print('LOADING_DINOV2',flush=True)
    model=AutoModel.from_pretrained('facebook/dinov2-base',use_safetensors=True).cuda().eval()
    ...
    manifest=pd.read_csv(BASE/'v1/inference_manifest.csv');evals=manifest[manifest.role=='source_fit'];assert len(evals)==1462
    rows=[]
    ...
    f=f.merge(labels,on='image_id',validate='one_to_one').merge(baseline,on='image_id',validate='one_to_one');f.to_csv(OUT/'evaluation_predictions.csv',index=False)
    rows=[];cis=[]
    for scope,data in [('all_source_fit',f),('tiny_only_plus_all_normal',f[(f.label==0)|(f.stratum=='tiny_only')])]:
        for category,g in data.groupby('category'):
            for method in ['D_z','dino_max','dino_top1']:rows.append(dict(scope=scope,category=category,method=method,auroc=roc_auc_score(g.label,g[method]),ap=average_precision_score(g.label,g[method])))
        for ci in intervals(data,[('dino_max','D_z'),('dino_top1','D_z')],replicates=2000):cis.append(dict(scope=scope,**ci))
