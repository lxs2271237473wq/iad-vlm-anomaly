...
BASE=ROOT/'results/stage25_tiny_defects';OUT=BASE/'dinov2_ad2_v1'

@torch.inference_mode()
def main():
    OUT.mkdir(parents=True,exist_ok=False);seed_all(42);torch.set_num_threads(4)
    (OUT/'protocol.json').write_text(json.dumps(dict(code_sha256=sha256(__file__),model='facebook/dinov2-base',input_size=518,reference_count=32,reference_selection='First 32 normal train image IDs per category; every patch retained',feature='L2 normalized last hidden patch tokens, exclude CLS',primary='fusion_frozen versus D_z; fixed maximum cosine distance for DINO score',secondary='mean largest 1% patch distances',scope='1084 AD2 public images; fixed source fusion; exploratory target evaluation',training=False,limitations='New backbone baseline only, not novel algorithm or exact AnomalyDINO reproduction; reference budget differs from original WRN50.'),indent=2))
    ...
    manifest=pd.read_csv(BASE/'v1/inference_manifest.csv');evals=manifest[manifest.role=='target_public_development'];assert len(evals)==1084
    rows=[]
    for category,g in evals.groupby('category',sort=True):
        ref=manifest[(manifest.dataset=='AD2')&(manifest.category==category)&(manifest.original_split=='train')].sort_values('image_id').head(32)
    ...
    baseline=pd.read_csv(RUN/'20260911_e/evaluation/predictions_with_evaluation_labels.csv')[['image_id','D512','scene_group']].rename(columns={'D512':'D_z'})
    ...
    rows=[];cis=[]
    for scope,data in [('AD2_public_all',f),('tiny_only_plus_all_normal',f[(f.label==0)|(f.stratum=='tiny_only')])]:
        eligible=data.groupby('category').label.nunique();eligible=eligible[eligible==2].index;data=data[data.category.isin(eligible)]
    ...
    (OUT/'complete.json').write_text(json.dumps(dict(status='complete',images=len(f),new_representation=True,new_algorithm=False,target_evaluated=True),indent=2));print(macro.to_string())
