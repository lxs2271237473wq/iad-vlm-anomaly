...
# The container is limited to ~7 CPUs by cgroup, but PyTorch defaults its intra-op
# thread count to the visible core count (64 here). That oversubscription makes the
# image transform ~18x slower (1.73 s -> 0.10 s per call). The thread count does not
# change any computed value. Override with SUPERAD_TORCH_THREADS if needed.
import torch as _torch
_torch.set_num_threads(int(os.environ.get("SUPERAD_TORCH_THREADS", "8")))
