"""Raw-order-preserving residual fusion audit of OrbitAD and SuperAD-Reg4.
...
OUT = ROOT/'orbitad/results/a54_raw_preserving_residual_fusion_v1'
METHODS = ('superad_reg4','aligned_residual_010','aligned_residual_025','aligned_residual_050')
...
def main():
    ...
    protocol={
        'role':'final exploratory public-development fusion audit; transfer required before promotion',
        'formulas_declared_before_metric_read':list(METHODS),
        'alignment':'OrbitAD multiplied by q99.5(SuperAD)/q99.5(OrbitAD), no clipping or subtraction; label-free',
        'fusion':'(1-lambda)*raw_SuperAD + lambda*aligned_OrbitAD for predeclared lambda 0.10/0.25/0.50',
    ...
    def values(row):
        ...
        qs=max(float(np.quantile(s,.995)),1e-12); qo=max(float(np.quantile(o,.995)),1e-12)
        aligned=o*(qs/qo)
        return {'superad_reg4':s,
                'aligned_residual_010':.90*s+.10*aligned,
                'aligned_residual_025':.75*s+.25*aligned,
                'aligned_residual_050':.50*s+.50*aligned}
    ...
    (OUT/'summary.json').write_text(json.dumps(payload,indent=2));(OUT/'A54_COMPLETE.json').write_text(json.dumps({'status':'complete'},indent=2))
