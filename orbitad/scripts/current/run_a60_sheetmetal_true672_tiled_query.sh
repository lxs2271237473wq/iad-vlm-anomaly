...
OUT="$ROOT/ad2_model_zoo/results/a60_superad_reg4_sheetmetal_true672_tiled_query_v1"
...
out=Path("$OUT")
(out/"A60_INFERENCE_COMPLETE.json").write_text(json.dumps({
  "status":"complete", "category":"sheet_metal", "reference_resolution":448,
  "local_query_resolution":672, "implementation_fix":"resize transform updated at runtime",
  "fusion":"0.90 global + 0.10 q99.5-aligned tiled residual",
  "comparison":"A57 used 448 local windows due stale transform; A60 isolates true resolution gain"
}, indent=2))
print("A60_INFERENCE_COMPLETE")
PY

UNIFIED_METHOD=superad_reg4_true672_tiled_query \
UNIFIED_LAYOUT=superad \
UNIFIED_CATEGORIES=sheet_metal \
UNIFIED_COMPLETION_MARKER=A60_INFERENCE_COMPLETE.json \
UNIFIED_MAP_ROOT="$OUT" \
UNIFIED_OUT="$ROOT/orbitad/results/a60_superad_reg4_sheetmetal_true672_tiled_query_eval_v1" \
