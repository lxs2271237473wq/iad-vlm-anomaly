"""A13: controlled true multi-input-scale DINOv3 memory experiment."""
...
OUT=ROOT/'orbitad/results/a13_true_multiscale_v1'
LAYERS=[2,5,8,11]
METHOD_SIZES={'full384':384,'full640':640}
...
def main():
    ...
    protocol={'model':'vit_base_patch16_dinov3.lvd1689m','layers':LAYERS,'methods':list(METHOD_SIZES),'input_sizes':METHOD_SIZES,'reused_external_scale':'full512 from a10_multilayer_v1','memory_per_image_per_layer':64,'memory_limit_per_layer':32768,'sampling':'fixed 8x8 spatial grid, paired across layers; seed20260913 cap','score':'mean over 4 layers of Euclidean NN distance / 768','augmentation':False,'postprocessing':False,'train':'train/good all','calibration':'validation/good','test':'public development only','weight_search':False,'official_reproduction':False,'budget_note':'matched memory count and architecture across input scales','code_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'upstream_model_sha256':hashlib.sha256(source.read_bytes()).hexdigest()}
    ...
    progress=[]
    for method,input_size in METHOD_SIZES.items():
        model=timm.create_model(protocol['model'],pretrained=True,num_classes=0,img_size=input_size).eval().cuda()
        ...
        def extract(path):
            with Image.open(path) as im:
                im=im.convert('RGB')
                im=im.resize((input_size,input_size),Image.Resampling.BICUBIC)
                x=to_tensor(im)[None].cuda()
            x=(x-mean)/std
            grid=input_size//16
            return [f.reshape(1,grid,grid,768).cpu().numpy() for f in backbone(x)]
        @torch.inference_mode()
    ...
    (OUT/'MAPS_COMPLETE.json').write_text(json.dumps({'categories':8,'methods':list(METHOD_SIZES),'status':'maps_complete'}))
