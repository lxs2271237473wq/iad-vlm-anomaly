...
A6_ROOT = Path("orbitad/results/a11_fixed_fusion_v1")
A6_ROOT.mkdir(parents=True, exist_ok=False)
PUBLIC_META = A10 / "public_meta.csv"
OUT_DETAIL = A6_ROOT / "native_category_condition.csv"
METHODS = ["pca16", "full512", "patch640", "pca_full_equal", "pca_patch_equal"]
BASES = ["pca16", "full512", "patch640"]
protocol = {
    "primary":"pca_patch_equal", "control":"pca_full_equal",
    ...
    "postprocessing":False, "architecture":"fixed arithmetic fusion baseline, not innovation"
}
...
class MapReader:
    ...
    def __getitem__(self,i):
        ...
        branch="full512" if self.method=="pca_full_equal" else "patch640"
        with Image.open(DATA_ROOT / row["image_path"]) as im: size=im.size
        parts=[cv2.resize(np.asarray(base_map(m,row),dtype=np.float32),size,interpolation=cv2.INTER_LINEAR)/scales[row["category"]][m] for m in ("pca16",branch)]
