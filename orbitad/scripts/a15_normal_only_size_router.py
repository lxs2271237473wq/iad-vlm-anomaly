...
A6_ROOT = Path("orbitad/results/a15_normal_only_size_router_v2")
A6_ROOT.mkdir(parents=True, exist_ok=False)
PUBLIC_META = A10 / "public_meta.csv"
OUT_DETAIL = A6_ROOT / "native_category_condition.csv"
METHODS = ["patch640", "full_patch_equal", "normal_only_size_router"]
BASES = ["full512", "patch640"]
protocol = {
    "primary":"normal_only_size_router", "controls":["patch640","full_patch_equal"],
    ...
    "routing_rule":"if largest supra-normal patch component >0.1% of score-grid area use equal full+patch; otherwise use patch",
    ...
    "postprocessing":False, "architecture":"normal-only hard size router pilot"
}
...
for row in validation:
    ...
    i=int(row["map_index"])
    a=np.load(A10 / "patch640" / row["category"] / "validation" / f"{i:05d}.npy")
    a=np.asarray(a,dtype=np.float32)/scales[row["category"]]["patch640"]
...
def routed_parts(row):
    ...
    max_ratio=0.0 if num<=1 else float(stats[1:,cv2.CC_STAT_AREA].max()/binary.size)
    route="full_patch_equal" if max_ratio>0.001 else "patch640"
    return patch,route,max_ratio
class MapReader:
    def __init__(self,method): self.method=method
    def __getitem__(self,i):
        row=row_lookup[i]
        if self.method in BASES: return base_map(self.method,row)
        patch,route,_=routed_parts(row)
        ...
        elif self.method=="normal_only_size_router":
            if route=="patch640":
                result=patch
            else:
                with Image.open(DATA_ROOT / row["image_path"]) as im: size=im.size
...
route_rows=[]
for row in rows:
    _,route,max_ratio=routed_parts(row)
    route_rows.append({"map_index":row["map_index"],"category":row["category"],"condition":row["condition"],"route":route,"max_component_ratio":max_ratio})
with (A6_ROOT/"route_decisions.csv").open("w",newline="") as f:
...
# Reproduce both A12 controls before accepting the router result.
previous=json.loads((A12 / "native_summary.json").read_text())
errors={m:abs(summary[m]["all_condition_category_macro"]-previous[m]["all_condition_category_macro"]) for m in ("patch640","full_patch_equal")}
