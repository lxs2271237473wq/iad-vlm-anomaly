...
A18 = Path("orbitad/results/a18_state_routed_memory_v1")
...
assert (A18 / "MAPS_COMPLETE.json").exists()
A6_ROOT = Path("orbitad/results/a18_state_routed_memory_eval_v1")
A6_ROOT.mkdir(parents=True, exist_ok=False)
PUBLIC_META = A18 / "public_meta.csv"
OUT_DETAIL = A6_ROOT / "native_category_condition.csv"
METHODS = ["state_patch640", "full_state_equal", "state_component_soft_router"]
BASES = ["full512", "state_patch640"]
protocol = {
    "primary":"state_component_soft_router", "controls":["state_patch640","full_state_equal"],
    "weights":[0.5,0.5], "weight_search":False,
    "routing_threshold":"per-category median validation/good patch q99.9 after A12 normalization",
    "routing_rule":"default equal fusion; within each supra-normal patch component, patch preference is exp(-component_area_ratio/0.001)",
    ...
    "postprocessing":False, "architecture":"A18 top-2 normal-state memory with component-level continuous soft router"
}
(A6_ROOT / "protocol.json").write_text(json.dumps(protocol,indent=2))
# Freeze scales before reading public labels or maps.
validation=list(csv.DictReader(open(A18 / "validation_meta.csv")))
a12_scales=json.loads((A12 / "frozen_scales.json").read_text())
scales={cat:{"full512":values["full512"]} for cat,values in a12_scales.items()}
state_scale_values=defaultdict(list)
threshold_values=defaultdict(list)
for row in validation:
    assert int(row["label"])==0 and "/validation/good/" in "/"+row["image_path"]
    i=int(row["map_index"])
    a=np.load(A18 / row["category"] / "state_patch640" / "validation" / f"{i:05d}.npy")
    state_scale_values[row["category"]].append(float(np.quantile(a,.95)))
for cat,values in state_scale_values.items():
    scales[cat]["state_patch640"]=max(float(np.median(values)),1e-12)
for row in validation:
    i=int(row["map_index"])
    a=np.load(A18 / row["category"] / "state_patch640" / "validation" / f"{i:05d}.npy")
    a=np.asarray(a,dtype=np.float32)/scales[row["category"]]["state_patch640"]
    assert np.isfinite(a).all()
    threshold_values[row["category"]].append(float(np.quantile(a,.999)))
route_thresholds={cat:float(np.median(values)) for cat,values in sorted(threshold_values.items())}
(A6_ROOT / "frozen_scales.json").write_text(json.dumps(scales,indent=2))
(A6_ROOT / "route_thresholds.json").write_text(json.dumps(route_thresholds,indent=2))
...
def base_map(method,row):
    i=row["map_index"]
    if method=="state_patch640":
        return np.load(A18 / row["category"] / "state_patch640" / "public" / f"{i:05d}.npy")
    return np.load(A10 / method / row["category"] / "public" / f"{i:05d}.npy")
def routed_parts(row):
    patch=np.asarray(base_map("state_patch640",row),dtype=np.float32)/scales[row["category"]]["state_patch640"]
...
class MapReader:
    ...
    def __getitem__(self,i):
        ...
        if self.method=="full_state_equal":
            ...
            result=0.5*(
                cv2.resize(full,size,interpolation=cv2.INTER_LINEAR)
                + cv2.resize(patch,size,interpolation=cv2.INTER_LINEAR)
            )
        elif self.method=="state_component_soft_router":
            with Image.open(DATA_ROOT / row["image_path"]) as im: size=im.size
            full=np.asarray(base_map("full512",row),dtype=np.float32)/scales[row["category"]]["full512"]
...
route_rows=[]
for row in rows:
    _,alpha,max_ratio=routed_parts(row)
    route_rows.append({"map_index":row["map_index"],"category":row["category"],"condition":row["condition"],"max_component_ratio":max_ratio,"mean_patch_preference":float(alpha.mean()),"max_patch_preference":float(alpha.max())})
with (A6_ROOT/"route_diagnostics.csv").open("w",newline="") as f:
    w=csv.DictWriter(f,fieldnames=list(route_rows[0])); w.writeheader(); w.writerows(route_rows)
EDGES={}
...
external_controls={
    "patch640":json.loads((A10/"native_summary.json").read_text())["patch640"]["all_condition_category_macro"],
    "full_patch_equal":json.loads((A12/"native_summary.json").read_text())["full_patch_equal"]["all_condition_category_macro"],
    "normal_only_component_soft_router":json.loads((A16/"native_summary.json").read_text())["normal_only_component_soft_router"]["all_condition_category_macro"],
}
(A6_ROOT / "external_controls.json").write_text(json.dumps(external_controls,indent=2))
...
print("EXTERNAL_CONTROLS",external_controls,flush=True)
