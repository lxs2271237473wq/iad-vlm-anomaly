...
\begin{abstract}
Industrial anomaly recognition with general-purpose vision-language models can be unreliable when defects are small, localized, and visually subtle.
...
This makes direct full-image VLM reasoning difficult for industrial inspection.
...
We use $S_{\mathrm{quality}}$ as the main method score.
The adaptive variant is reported only as a refinement.
...
It does not claim pixel-level segmentation SOTA or general defect-cause interpretation.
EfficientAD is evaluated under a fixed 30-epoch budget, AnomalyCLIP~\cite{Zhou2024AnomalyCLIP} is not experimentally included, and adaptive consistency has only a small gain over quality-only.
...
Overall, candidate-quality calibration makes localization-guided VLM evidence more reliable for image-level anomaly recognition.
